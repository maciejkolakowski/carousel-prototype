...
carousel.prototype.changeSlide = function(changeType){
    ...
    if(!this.cssTransitions) {
        TweenLite.to(activeSlide, transformedSpeed, {opacity: 0});
    }
    ...
    switch(changeType) {
        ...
        case 'next':
                if(activeSlide.nextElementSibling) {
                    ...
                    if(!this.cssTransitions) {
                        TweenLite.to(activeSlide.nextElementSibling, transformedSpeed, {opacity:1});
                    }
                } else {
                    this.slides[0].classList.add(this.activeClassName);
                    if(!this.cssTransitions) {
                        TweenLite.to(this.slides[0], transformedSpeed, {opacity:1});
                    }
                }
            break;
        ...
        case 'prev':
                if(activeSlide.previousElementSibling) {
                    ...
                    if(!this.cssTransitions) {
                        TweenLite.to(activeSlide.previousElementSibling, transformedSpeed, {opacity:1});
                    }
                } else {
                    this.slides[this.slides.length-1].classList.add(this.activeClassName);
                    if(!this.cssTransitions) {
                        TweenLite.to(this.slides[this.slides.length], transformedSpeed, {opacity:1});
                    }
                }
                break;
